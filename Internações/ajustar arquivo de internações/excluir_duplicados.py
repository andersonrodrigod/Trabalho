...
mascara_cd_dup = df_mantidos["CD_USUARIO"].duplicated(keep=False)
mascara_proc_exato = df_mantidos.duplicated(
    subset=["CD_USUARIO", "PROCEDIMENTO"], keep="first"
)
...
mascara_cd_dup_2 = df_mantidos["CD_USUARIO"].duplicated(keep=False)
mascara_p1_repete = df_mantidos.duplicated(
    subset=["CD_USUARIO", "_P1_PROC"], keep="first"
)
