...
# Carrega os arquivos
df_disparo = pd.read_excel("complica_outubro_hap_base_incorreta.xlsx")
...
print("🔗 Procurando IDADE T da planilha geral usando o Código...")

# Merge usando o CÓDIGO como chave
df_merge = df_disparo.merge(
    df_geral[["USUARIO", "IDADE T"]],
    left_on="Nome",
    right_on="USUARIO",
    how="left"
)

print("📝 Criando coluna DT ADESAO final...")

# Se não achar data, coloca 'NÃO ENCONTRADO'
df_merge["IDADE T"] = df_merge["IDADE T"].fillna("NÃO ENCONTRADO")

# Remove coluna COD USUARIO duplicada
df_merge = df_merge.drop(columns=["USUARIO"])

# Salvar resultado final
df_merge.to_excel("complica_outubro_hap_base_incorreta_id.xlsx", index=False)

print("💾 Arquivo salvo como 'complica_outubro_hap_base_incorreta.xlsx'!")
